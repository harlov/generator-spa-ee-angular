...
ModalGenerator.prototype.askFor = function askFor() {
    ...
    ccUtils.addNamePrompt(this, prompts, 'modal');

    this.prompt(prompts, function(props) {
        if(props.name) {
            this.name = props.name;
        }
        ccUtils.askForModuleAndDir('modal', this, true, cb);
    }.bind(this));
...
ModalGenerator.prototype.files = function files() {
    ...
    ccUtils.processTemplates(this.name, ccUtils.getDirWithSrc(this.dir), 'modal', this, null, null, this.module);

    setTimeout((function() {

        console.log('');
        console.log('  Open this modal by using ' + chalk.bold('angular-ui-bootstrap') + ' module\'s ' +
        chalk.bold('$modal') + ' service:');
        console.log('');
